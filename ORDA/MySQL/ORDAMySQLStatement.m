...
- (id)initWithGovernor:(id<ORDAGovernor>)governor withSQL:(NSString *)SQL
{
	...
	if (![governor isKindOfClass:[ORDAMySQLGovernor class]])
		return (ORDAMySQLStatement *)[ORDAMySQLErrorResult errorWithCode:kORDAInternalAPIMismatchErrorResultCode].retain;
	ORDAMySQLGovernor * mysql_gov = (ORDAMySQLGovernor *)governor;
	
	_result = nil;
	
	_statement = mysql_stmt_init(mysql_gov.connection);
	if (!self.statement)
		return (ORDAMySQLStatement *)[ORDAMySQLErrorResult errorWithCode:(ORDACode)kORDANoMemoryErrorResultCode andMySQL:mysql_gov.connection].retain;
	...
	if (status)
		return (ORDAMySQLStatement *)[ORDAMySQLErrorResult errorWithMySQL:mysql_gov.connection].retain;
...
- (id<ORDAResult>)bindNullToIndex:(int)index
{
	if (index >= self.bindCount)
		return (id<ORDAResult>)[ORDAMySQLErrorResult errorWithCode:kORDABadBindIndexErrorResultCode].retain;
